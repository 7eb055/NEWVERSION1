-- SQL Migration: Add Settings Functionality Columns
-- Run this to add the necessary columns for the Settings page functionality
-- PostgreSQL Database

-- Add settings columns to Users table (one at a time for compatibility)
...
-- Update password_changed_at for existing users (set to created_at if null)
UPDATE Users 
SET password_changed_at = created_at 
WHERE password_changed_at IS NULL;

-- Create index for better performance on deleted accounts
CREATE INDEX IF NOT EXISTS idx_users_is_deleted ON Users(is_deleted);

-- Create index for profile visibility filtering
CREATE INDEX IF NOT EXISTS idx_users_profile_visibility ON Users(profile_visibility);

-- Ensure notification_preferences column exists in Attendees table (it should already exist)
-- This column stores JSON data for notification preferences
