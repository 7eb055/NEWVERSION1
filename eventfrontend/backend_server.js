...
pool.connect((err, client, release) => {
  if (err) {
    ...
    console.log('G£à Connected to PostgreSQL database');
    release();
  }
});
...
const authorizeOrganizer = async (req, res, next) => {
  try {
    ...
    if (userQuery.rows.length === 0 || userQuery.rows[0].role !== 'organizer') {
      return res.status(403).json({ message: 'Forbidden: Organizers only' });
    }

    req.organizer = userQuery.rows[0];
    next();
  } catch (error) {
...
app.get('/api/admin/dashboard-stats', authenticateToken, authorizeAdmin, async (req, res) => {
  try {
    ...
    const organizers = await pool.query('SELECT COUNT(*) FROM users WHERE role = \'organizer\'');
    const attendees = await pool.query('SELECT COUNT(*) FROM users WHERE role = \'attendee\'');
    
    // Get revenue from payments table
    let totalRevenue = 0;
    try {
      const revenue = await pool.query('SELECT COALESCE(SUM(amount), 0) as total_revenue FROM payments WHERE status = \'completed\'');
      totalRevenue = parseFloat(revenue.rows[0].total_revenue) || 0;
    } catch (err) {
      console.log('Error getting revenue:', err.message);
      totalRevenue = 0;
    }
...
app.get('/api/admin/events', authenticateToken, authorizeAdmin, async (req, res) => {
  try {
    ...
    let query = `
      SELECT e.id as event_id, e.title as event_name, e.organizer_id, e.status, e.start_date as event_date, 
             e.ticket_price, e.capacity as max_attendees, e.created_at, e.description, e.venue, e.location,
    ...
    if (search) {
      conditions.push('e.title ILIKE $' + (params.length + 1));
      params.push(`%${search}%`);
    }
    ...
    query += ` 
      GROUP BY e.id, e.title, e.organizer_id, e.status, e.start_date, e.ticket_price, e.capacity, e.created_at, e.description, e.venue, e.location, u.name 
...
app.get('/api/admin/reports', authenticateToken, authorizeAdmin, async (req, res) => {
  try {
    ...
    if (type === 'overview' || type === 'all') {
      ...
      // Revenue by day (using payments table)
  ...
  try {
    ...
    const eventsQuery = await pool.query(`
      SELECT e.id as event_id, e.title as event_name, e.start_date as event_date, 
             ...
             e.status, e.created_at, e.description, e.venue, e.location, e.image,
             u.name as organizer_name, u.company_name,
             COUNT(t.id) as registration_count
      FROM events e
      LEFT JOIN users u ON e.organizer_id = u.id
      LEFT JOIN tickets t ON e.id = t.event_id
      WHERE e.status = $1
      GROUP BY e.id, e.title, e.start_date, e.ticket_price, e.capacity, e.status, e.created_at, e.description, e.venue, e.location, e.image, u.name, u.company_name
  ...
  try {
    ...
    const eventQuery = await pool.query(`
      SELECT e.id as event_id, e.title as event_name, e.start_date as event_date, 
             e.ticket_price, e.capacity as max_attendees, 
             e.status, e.created_at, e.description, e.venue, e.location, e.image,
             u.name as organizer_name, u.company_name, u.phone as organizer_phone,
             COUNT(t.id) as registration_count
      FROM events e
      LEFT JOIN users u ON e.organizer_id = u.id
      LEFT JOIN tickets t ON e.id = t.event_id
      WHERE e.id = $1
      GROUP BY e.id, e.title, e.start_date, e.ticket_price, e.capacity, e.status, e.created_at, e.description, e.venue, e.location, e.image, u.name, u.company_name, u.phone
    `, [eventId]);
...
app.listen(PORT, () => {
  ...
  console.log(`=ƒîì Environment: ${process.env.NODE_ENV || 'development'}`);
  
  // Email notification scheduler is temporarily disabled
  console.log('=ƒôº Email notification scheduler is disabled');
  console.log('=ƒÆí To re-enable: uncomment notification scheduler code in server.js');
  
  // Commented out notification scheduler
  // if (process.env.NODE_ENV === 'production' || process.env.ENABLE_NOTIFICATIONS === 'true') {
  //   notificationScheduler.start();
  //   console.log('=ƒôº Email notification scheduler started');
  // } else {
  //   console.log('=ƒôº Email notification scheduler disabled (set ENABLE_NOTIFICATIONS=true to enable)');
  // }
});

// Graceful shutdown (notification scheduler disabled)
process.on('SIGTERM', () => {
  console.log('=ƒ¢æ SIGTERM received, shutting down gracefully...');
  // notificationScheduler.stop(); // Disabled
  process.exit(0);
});

process.on('SIGINT', () => {
  console.log('=ƒ¢æ SIGINT received, shutting down gracefully...');
  // notificationScheduler.stop(); // Disabled
  process.exit(0);
});
