...
const OrganizerDashboard = () => {
  ...
  const calculateSalesData = useCallback((eventsData) => {
    try {
      console.log('📊 Calculating sales data from events:', eventsData);
      
      // Calculate sales data from provided events
      let totalIncome = 0;
      const eventSales = [];
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      // Calculate sales data immediately after events are loaded
      calculateSalesData(loadedEvents);
  ...
  useEffect(() => {
    ...
    if (userData) {
      ...
      loadEvents(); // This will also calculate sales data
      loadCompanies();
      loadAttendees();
    } else {
      // No authenticated user - set default user info and empty data
