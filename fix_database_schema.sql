...
-- Make sure log_id exists and is the primary key
ALTER TABLE attendancelogs DROP CONSTRAINT IF EXISTS attendancelogs_pkey;
-- If log_id doesn't exist, add it
DO $$
BEGIN
...
-- You may need to adjust these based on your exact database schema
...
UPDATE organizers SET organizer_name = 'Unknown Organizer' WHERE organizer_name IS NULL;

-- Repair events missing names
UPDATE events SET event_name = 'Unnamed Event #' || event_id WHERE event_name IS NULL;
